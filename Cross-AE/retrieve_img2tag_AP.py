...
AP, number_of_tags = utils.CalcAP_Img2Tag(img_features, tag_features, imp_list, tag_paths)
#----------mAP----------
...
#----------csvの保存----------
rows = []
rows.append(["font", "AP", "number of tags"])
for f in range(len(font_paths)):
    font_name = os.path.basename(font_paths[f])[:-4]
    rows.append([font_name, AP[f], number_of_tags[f]])
rows = np.asarray(rows)
with open(f"{DIR_PATH}/evaluate/retrieve_img2tag_AP_{DATASET}.csv", 'w', newline='', encoding='utf-8') as file:
