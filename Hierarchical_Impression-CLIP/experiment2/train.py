...
def train(dataloader, models, criterion, optimizer, epoch, total_itr, logger, save_folder):
    ...
    for idx, data in enumerate(dataloader):
        ...
        with torch.set_grad_enabled(True):
            ...
            # culuculate loss
            loss_img = criterion(features, img_labels)
            loss_tag = criterion(features, tag_labels)
        ...
        csv_writer.writerow([total_itr, loss_img.item(), loss_tag.item(), loss.item()])
        f.close()
        total_itr += 1
    logger.add_scalar('loss_img_epoch', np.mean(loss_img_list), epoch)
    logger.add_scalar('loss_tag_epoch', np.mean(loss_tag_list), epoch)
    logger.add_scalar('loss_epoch', np.mean(loss_list), epoch)
    return np.mean(loss_img_list), np.mean(loss_tag_list), np.mean(loss_list), total_itr


if __name__ == '__main__':
    # define constant
    SAVE_FOLDER = "Hierarchical-CLIP/results_non"
