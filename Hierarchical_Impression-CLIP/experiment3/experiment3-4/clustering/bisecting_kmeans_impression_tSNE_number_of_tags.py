...
EXP = utils.EXP
DATASET = 'train'
SAVE_DIR = f'{EXP}/clustering/cluster_visualization/tSNE_number_of_tags/{DATASET}'
...
# tSNE
tSNE_filename = f'{EXP}/clustering/cluster_visualization/tSNE/train/impression_cluster_tSNE_model.pkl'
...
tag_cluster_id = number_of_tags
number_of_clusters = max(tag_cluster_id)

# プロット(マウスオーバーで画像と印象タグを表示)
fig, ax = plt.subplots()
patches = [mpatches.Patch(color=plt.cm.tab10(i), label=f'{i+1}') for i in range(number_of_clusters)]
sc = plt.scatter(X, Y, c=plt.cm.tab10(np.asarray(tag_cluster_id-1, dtype=np.int64)), 
