...
class DMH_D(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in index:
            ...
            img_label = [self.img_hierarchy[i], i, i]
            tag_label = [self.tag_hierarchy[i], i, i]
            imgs.append(img)
...
class DMH_BS(Sampler):
    ...
    def __iter__(self):
        ...
        while len(remaining) > self.batch_size:
            ...
            # 画像の階層構造と印象の階層構造で同じものを持ってこないようにする(visitedを更新しているから大丈夫か)
        ...
        # メモ: ↓いろいろおかしい気がする，条件に応じてあまりも入れたい？
        # 現状だとここに入ることはない
