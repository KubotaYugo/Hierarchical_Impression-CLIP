...
def main():
    ...
    tag_cluster_id = np.load(TAG_HIERARCHY_PATH)["arr_0"].astype(np.int64)
    ...
    # クラスタ別に印象タグを保存
    for i in range(number_of_clusters):
        SAVE_DIR = f"{EXP}/LR={LR}, BS={BATCH_SIZE}/cluster_tags"
    ...
    SAVE_DIR = f"{EXP}/LR={LR}, BS={BATCH_SIZE}/cluster_number_of_tags"
    os.makedirs(SAVE_DIR, exist_ok=True)
    ...
    # タグの種類別の頻度をプロット
    SAVE_DIR = f"{EXP}/LR={LR}, BS={BATCH_SIZE}/cluster_frequency_of_tags"
    os.makedirs(SAVE_DIR, exist_ok=True)
    for i in range(number_of_clusters):
    ...
    # タグの割合を保存
    SAVE_DIR = f"{EXP}/LR={LR}, BS={BATCH_SIZE}/cluster_frequency_rate_of_tags"
    os.makedirs(SAVE_DIR, exist_ok=True)
    for i in range(number_of_clusters):
