'''
baselineとaverage, iterative, label_andをそれぞれ比較し, 
RR_tag2imgとRR_imgtagの双方で良くなったものtop10と悪くなったものtop10のフォントを保存する
RR_tag2imgとRR_imgtagは各モデルのシード値について平均
'''
...
def save_fonts(indexes, order):
    for i in range(comparison.shape[0]):
        ...
        for index in indexes[i]:
            ...
            RR_tag2img_baseline = average_RR_list['tag2img'][0][index]
            RR_tag2img_comparison = average_RR_list['tag2img'][i+1][index]
            RR_tag2img_difference = RR_tag2img_comparison-RR_tag2img_baseline
            RR_img2tag_baseline = average_RR_list['img2tag'][0][index]
            RR_img2tag_comparison = average_RR_list['img2tag'][i+1][index]
...
# loss_type, weights
param_set = [['average',   [1.0, 0.0, 0.0,]],
             ['average',   [1.0, 1.0, 1.0,]],
             ['iterative', [1.0, 1.0, 1.0,]],
             ['label_and', [1.0, 1.0, 1.0,]]]
model_name = ['baseline', 'average', 'iterative', 'label_and']
random_seeds = [1, 2, 3, 4, 5]

# ディレクトリの作成
K = 10
SAVE_DIR = f'{EXPT}/compare_models/retrieval_rank_save_font/{DATASET}/K={K}'
os.makedirs(SAVE_DIR, exist_ok=True)

# retrieval rankの計算
# RR_list[i][j]: シード値i+1のときのparam_set[j]のRR_tag2imgとRR_img2tag
RR_list = []
for random_seed in random_seeds:
    RR = [get_RR(params, param_set_i+[random_seed]) for param_set_i in param_set]
    RR_list.append(RR)

# シード値を通して平均
average_RR_list = {'tag2img':[], 'img2tag':[]}
for i in range(len(param_set)):
    temp_dict = {}
    for direction in ['tag2img', 'img2tag']:
        average_RR_list_temp = [RR_list[random_seed-1][i][direction] for random_seed in random_seeds]
        average_RR_list_temp = np.asarray(average_RR_list_temp)
        average_RR = np.average(average_RR_list_temp, axis=0)
        average_RR_list[direction].append(average_RR)
...
for direction in ['tag2img', 'img2tag']:
    ...
    average_RR_list_direction = np.asarray(average_RR_list[direction])
    baseline = average_RR_list_direction[0]
    comparison = average_RR_list_direction[1:]
