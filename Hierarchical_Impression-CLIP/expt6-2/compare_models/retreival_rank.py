...
# ディレクトリの作成
SAVE_DIR = f'{EXPT}/compare_models/retrieval_rank/{DATASET}'
...
for i in range(len(param_set)):
    ...
    for direction in ['tag2img', 'img2tag']:
        ...
        temp_dict[direction] = average_RR
    average_RR_list.append(temp_dict)
...
for direction in ['tag2img', 'img2tag']:
    ...
    for i in range(len(param_set)):
        for j in range(len(param_set)):
                x = average_RR_list[i][direction]
                y = average_RR_list[j][direction]
