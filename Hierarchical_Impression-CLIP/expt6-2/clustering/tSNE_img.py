...
SAVE_DIR = f'{EXPT}/clustering/tSNE/img/{DATASET}'
os.makedirs(SAVE_DIR, exist_ok=True)


# 学習データでtSNE
img_feature_path_train = f'{EXPT}/feature/img_feature/train.pth'
img_feature_train = torch.load(img_feature_path_train).to('cpu').detach().numpy()
tSNE_filename = f'{EXPT}/clustering/tSNE/img/tSNE_model.pkl'
if os.path.exists(tSNE_filename):
    ...
    print('tSNE start')
    tSNE = TSNE(initialization='pca', metric='euclidean', n_jobs=-1, random_state=7).fit(img_feature_train)
    with open(tSNE_filename, 'wb') as f:
        pickle.dump(tSNE, f)
    print('tSNE end')
    print('Calculated and saved new tSNE.')

# 画像特徴の取得 & tSNE embedding
...
if os.path.exists(tSNE_feature_filename):
    ...
    embedding = tSNE.transform(img_feature)
