...
mAP_cluster_tag2img = calc_mAP(RR_tag2img, img_cluster)         # 同じクラスタの画像を正例としたときのmAP(印象->画像)
mAP_cluster_img2tag = calc_mAP(RR_img2tag, tag_cluster)         # 同じクラスタの印象を正例としたときのmAP(画像->印象)
mAP_cluster_mean = (mAP_cluster_tag2img+mAP_cluster_img2tag)/2  # 上2つの平均

img_tag_cluster = img_cluster*10+tag_cluster                    # 画像クラスタ, 印象クラスタともに同じものを1つのクラスタとしたラベル
mAP_both_tag2img = calc_mAP(RR_tag2img, img_tag_cluster)        # 画像も印象も同じクラスタのフォントを正例としたときのmAP(印象->画像)
mAP_both_img2tag = calc_mAP(RR_img2tag, img_tag_cluster)        # 画像も印象も同じクラスタのフォントを正例としたときのmAP(画像->印象)
mAP_both_mean = (mAP_both_tag2img+mAP_both_img2tag)/2           # 上2つの平均
...
print(f'mAP_tag2img: {mAP_cluster_tag2img:.4f}')
print(f'mAP_img2tag: {mAP_cluster_img2tag:.4f}')
print(f'mAP_mean:    {mAP_cluster_mean:.4f}')

print('-----画像も印象も同じクラスタのフォントを正解としたとき-----')
print(f'mAP_tag2img: {mAP_both_tag2img:.4f}')
print(f'mAP_img2tag: {mAP_both_img2tag:.4f}')
print(f'mAP_mean:    {mAP_both_mean:.4f}')