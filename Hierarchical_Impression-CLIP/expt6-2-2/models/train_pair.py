...
'''
trainで学習した重みを初期値としてloss_pairを使って学習
'''
...
def main(params):
    with wandb.init(dir=f'{params.expt}/co-embedding'):
        ...
        SAVE_DIR = f'{EXPT}/co-embedding/{RUN_NAME}/results_pair'
        ...
        # load parameters
        model_path = f'{EXPT}/co-embedding/{RUN_NAME}/results/model/best.pth.tar'
        model_params = torch.load(model_path)
        emb_img.load_state_dict(model_params['emb_img'])
        emb_tag.load_state_dict(model_params['emb_tag'])
        ...
        for epoch in range(1, MAX_EPOCH + 1):
            ...
            loss = train_utils.train_pair(trainloader, emb_img, emb_tag, temperature, optimizer)
            ...
            print(f'[train]                     {train_utils.loss_pair_format_train(loss)}')
            ...
            parameters_to_save = {
                    'epoch':                                           epoch,
                    'train_loss_pair_total':                           loss['total'],
                    'train_loss_pair_img2tag':                         loss['img2tag'],
                    'train_loss_pair_tag2img':                         loss['tag2img'], 
...
if __name__ == '__main__':
    ...
    sweep_id = wandb.sweep(sweep=sweep_configuration, project='Hierarchical_ImpressionCLIP_6-2-2_pair')
    wandb.agent(sweep_id, function=lambda: main(params))

# python programs/Hierarchical_Impression-CLIP/expt6-2-2/models/train_pair.py