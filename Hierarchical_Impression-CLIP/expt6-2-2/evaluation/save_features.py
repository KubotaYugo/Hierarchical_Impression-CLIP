...
def extract_feature(dataloader, emb_img, emb_tag):
    img_feature_list = []
    tag_feature_list = []
...
emb_img.load_state_dict(params['emb_img'])
emb_tag.load_state_dict(params['emb_tag'])

# dataloderの準備
dataset = HierarchicalDataset.DatasetForEval(DATASET, EXPT, TAG_PREPROCESS)
...
# 画像特徴，印象特徴(複数タグ)の保存
_, _, embedded_img_feature, embedded_tag_feature = extract_feature(dataloader, emb_img, emb_tag)
torch.save(embedded_img_feature, EMBEDDED_IMG_FEATURE_PATH)
torch.save(embedded_tag_feature, EMBEDDED_TAG_FEATURE_PATH)

# 印象特徴(タグ単体)の保存
_, embedded_single_tag_feature = extract_single_tag_feature(singletagloader, emb_tag)
