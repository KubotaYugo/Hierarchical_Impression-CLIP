...
#         prompt2 = prompt2 + ", and " + tags[i+1] + ", respectively."                
#         prompt = prompt1 + prompt2
#     tokenized_tags = tokenizer(prompt, return_tensors="pt", 
...
class HierarchicalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        imgs, prompts, img_labels, tag_labels = [], [], [], []
        for i in idx:
            # get img and tokenized tags
            img = np.load(self.img_paths[i])["arr_0"].astype(np.float32)
            img = torch.from_numpy(img/255)
            tags = utils.get_font_tags(self.tag_paths[i])
        ...
        return torch.stack(imgs), torch.stack(prompts), torch.tensor(img_labels), torch.tensor(tag_labels)
...
class HierarchicalBatchSampler(Sampler):
    ...
    def get_label_split_by_index(self, idx, mode):
        if mode=='img':
            cluster = self.dataset.img_hierarchical_labels[idx]
        elif mode=='tag':
            cluster = self.dataset.img_hierarchical_labels[idx]
        font = idx
        return cluster, font
    ...
    def __iter__(self):
        ...
        for num in range(number_of_iteration):
            ...
            img_cluster, img_font = self.get_label_split_by_index(idx, 'img')
            ...
            tag_cluster, tag_font = self.get_label_split_by_index(idx, 'tag')
