...
# ディレクトリの作成
SAVE_DIR = f'{BASE_DIR}/tSNE_cluster'
...
for i, label in enumerate(np.unique(tag_cluster_id)):
    ...
    plt.scatter(X_tag[mask], Y_tag[mask], label=f'{label}_tag', color=plt.cm.tab20(i*2), 
                alpha=0.8, edgecolors='w', linewidths=0.1, s=5)
    plt.scatter(X_img[mask], Y_img[mask], label=f'{label}_img', color=plt.cm.tab20(i*2+1), 
                alpha=0.8, edgecolors='w', linewidths=0.1, s=5)
plt.legend()
