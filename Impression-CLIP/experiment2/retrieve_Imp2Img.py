"""
印象(testにある組み合わせで複数タグ)をクエリ，キーを画像として検索
...
#-----------保存用フォルダの準備----------
SAVE_PATH = f"{DIR_PATH}/Impression-CLIP/evaluate/retrieve_Imp2Img/{DATASET}"
...
for t in range(len(text_embedded_features)):
    
    #----------クエリの印象が持つタグの取得-----------
    query_tags = eval_utils.GetFontTags(tag_paths[t])
    
    write_row = [[] for i in range(K)]
    for k in range(K):
        #----------キーのフォントが持つタグの取得----------
        key_tags = eval_utils.GetFontTags(tag_paths[logits_topk_args[k][t]])
    ...
    #----------画像とcsvの保存----------
    font_name = os.path.basename(font_paths[t])[:-4]
