...
def val(epoch, models, criterion, dataloader, device):
    ...
    # ARRの計算
    ARR_tag2img = np.mean(retrieval_rank(matrix, "tag2img"))
    ARR_img2tag = np.mean(retrieval_rank(matrix, "img2tag"))
    meanARR = (ARR_tag2img+ARR_img2tag)/2
    
    #----------ロスの表示----------
    pprint(f"[{epoch}] Epoch: {epoch}, loss: {loss.item()}.")
    pprint(f"[{epoch}] Epoch: {epoch}, loss without temp: {loss_without_temp.item()}.")
        
    return loss.item(), loss_without_temp.item(), temp.t.item(), ARR_tag2img, ARR_img2tag, meanARR