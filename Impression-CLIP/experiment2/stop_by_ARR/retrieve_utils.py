...
def retrieve_imp_img(img_features, tag_features, img_paths, tag_paths, direction, mode, k_max, save_dir, dataset):
    # calcurate topk args
    ...
    os.makedirs(f'{save_dir}/retrieve_{direction}/{dataset}', exist_ok=True)
    for t in range(len(tag_features)):
        ...
        imsave(f"{save_dir}/retrieve_{direction}/{dataset}/{font_name}_{mode}.png", output_images, cmap=cm.gray)
        with open(f"{save_dir}/retrieve_{direction}/{dataset}/{font_name}_{mode}.csv", 'w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            writer.writerows(write_rows)


def retrieve_tag2img(img_features, single_tag_features, img_paths, tag_paths, tag_list, mode, k_max, save_dir, dataset):
    # calcurate topk args
    ...
    os.makedirs(f'{save_dir}/retrieve_tag2img/{dataset}', exist_ok=True)
    for t in range(len(tag_list)):
        ...
        # 画像とcsvの保存
        imsave(f"{save_dir}/retrieve_tag2img/{dataset}/{tag_list[t]}_{mode}.png", output_images, cmap=cm.gray)
        with open(f"{save_dir}/retrieve_tag2img/{dataset}/{tag_list[t]}_{mode}.csv", 'w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            writer.writerows(write_rows)


def retrieve_img2tag(img_features, single_tag_features, img_paths, tag_paths, tag_list, save_dir, dataset):
    # calcurate topk args
    similarity_matrix = torch.matmul(img_features, single_tag_features.T).to("cpu").detach().numpy()
    similarity_topk_args = np.argsort(-similarity_matrix, axis=1)  # 降順にするため，-similarityに
    
    os.makedirs(f'{save_dir}/retrieve_img2tag/{dataset}', exist_ok=True)
    for f in range(len(img_paths)):
        ...
        with open(f"{save_dir}/retrieve_img2tag/{dataset}/{font_name}.csv", 'w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            writer.writerows(write_rows)